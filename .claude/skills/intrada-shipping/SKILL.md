...
**Open the PR through `just pr-open`** (`-- --draft --title "..." --body "..."`),
not a bare `gh pr create`: it refuses when an issue number in the title has no
claim naming the current branch (#1702), which is the second way #1694 got
built twice on 2026-09-11 (the first is skipping `just claim` itself).
