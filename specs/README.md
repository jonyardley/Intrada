...
| [`background-audio-plugin.md`](background-audio-plugin.md) | Written as a Tauri plugin under `crates/intrada-mobile`, which no longer exists. The reference `BackgroundAudioPlugin.swift` was deleted in PENDING_COMMIT_HASH (#1745) and is recoverable from git history. Useful as reference for a native port, not implementable as written |
| [`live-activity-plugin.md`](live-activity-plugin.md) | Same: an ActivityKit design as a Tauri plugin in a deleted crate. Its reference Swift was deleted in PENDING_COMMIT_HASH (#1745) and is recoverable from git history. A native lock-screen spec does not exist yet |
