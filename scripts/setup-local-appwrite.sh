...
if ! docker compose version &> /dev/null; then
    echo -e "${RED}❌ docker compose not found. Please install Docker with Compose V2.${NC}"
    exit 1
fi
...
while [ $elapsed -lt $timeout ]; do
    if curl -s http://localhost/v1/health/version > /dev/null 2>&1; then
...
# Note: Project creation requires authentication
echo -e "${YELLOW}🏗️  Setting up development project...${NC}"
echo -e "${BLUE}ℹ️  For local development, please:${NC}"
echo -e "  1. Open Appwrite console at http://localhost/console"
echo -e "  2. Create a new project with ID: $APPWRITE_PROJECT_ID"
echo -e "  3. Create an API key with database permissions"
echo -e "  4. Update the .env.local file with your API key"
echo -e ""

# Set a placeholder API key for now
...
if [ -f "iOS/Intrada/Config.plist.template" ]; then
    if [ ! -f "iOS/Intrada/Config.plist" ]; then
        ...
        sed -i.bak "s|https://your-appwrite-endpoint.cloud.appwrite.io/v1|http://localhost/v1|g" iOS/Intrada/Config.plist
