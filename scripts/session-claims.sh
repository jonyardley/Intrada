...
# (#1703): a hand-written brief describes the picture as it was when drafted,
# and by the time the opener is pasted it has usually moved. Cached for a
# few minutes so a burst of session starts doesn't hit the API each time;
# silent (no output, exit 0) whenever gh, jq or the network aren't available,
# since a SessionStart hook failing loud is worse than one saying nothing.
...
while IFS= read -r row; do
  ...
  branch="$(grep -oE '`[^`]+`' <<<"$claim_body" | head -1 | tr -d '`' || true)"
