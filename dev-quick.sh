...
echo "⚡ Quick starting Intrada development environment (skipping type generation)"
...
# Function to build and run server (quick build)
start_server() {
    echo "🔄 Quick building and starting server..."
    cd server
...
# Function to build and run iOS app (if on macOS) - quick mode
start_ios() {
    ...
    echo "🔄 Quick building and launching iOS app (using existing bindings)..."
    cd iOS
    
    # Check if Xcode project exists, generate if needed
    if [ ! -f "Intrada.xcodeproj/project.pbxproj" ]; then
...
# Main execution
echo "📋 Quick starting Intrada development environment..."
echo "⚠️  Note: This skips type generation and full rebuilds for faster startup"
echo "    If you've made changes to shared types, run 'xt start' instead"
echo ""
...
echo "   xt start         # Full start with type generation"
echo ""